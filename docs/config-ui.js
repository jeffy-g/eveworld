...
preset:"Default",close:!1,remembered:{Default:{0:Util.deepClone(WorldConfig)}}}});let g;l.remember(WorldConfig),
l.add(WorldConfig,"helperMode").onChange(e=>{o("cameraHelper"),e&&g.getValue()&&g.setValue(!e),
...
l.add(WorldConfig,"resetCamera"),l.add(WorldConfig,"updateRegionColors");const c=l.addFolder("World Config#1");{
function f(e,o){const n=r.getObjectByName(a);n&&n[e](...o)}
c.add(WorldConfig,"Ray threshold(LY)").min(.001).max(1).step(.001).onChange((function(e){
d.params.Points.threshold=e*UniverseUnit.LY
})),c.add(WorldConfig,"system size(fixed)").min(1).max(50).step(.1).onChange((function(e){EVEWorld.setSystemSize(e)})),
c.add(WorldConfig,"system size(LY)").min(.25).max(3).step(.01).onChange((function(e){EVEWorld.setSystemSize(e,!0)}))
;const e=c.add(WorldConfig,"hunter radius(LY)").min(.01).max(3).step(.01).onChange((function(e){
f("setRadius",[e*UniverseUnit.LY])}));c.add(WorldConfig,"hunter opacity").min(0).max(1).step(.01).onChange((function(e){
f("setOpacity",[e])})),c.add(WorldConfig,"hunter sphere division").min(3).max(128).step(1).onChange((function(o){
f("setRadius",[e.getValue()*UniverseUnit.LY,o])})),c.add(WorldConfig,"hunter use wireframe").onChange((function(e){
f("useWireFrame",[e])})),c.add(WorldConfig,"closeUp to").min(.5).max(50).step(.01),
c.add(WorldConfig,"fov").min(30).max(180).step(1).onChange((function(e){i.fov=e,i.updateProjectionMatrix()}))}
...
;g=u.add(a,"Region name").onChange((function(e){forEachRegioins(o=>{n(o,e)})})),
...
const o=i.test(t),l=o?a.wormhole:a.universe;l[t]=!0;const g=(o?d:r).add(l,t).onChange(o=>{const a=e[t];a&&(a.visible=o,
n(a,o))});o&&s.push(g)}}const C=l.addFolder("Effect Sound");{const o=AppEffects.getDefaultTagList()
...
$queryAll("li.cr.boolean,li.cr.number").forEach(e=>{let o="";switch(e.textContent){case"helperMode":
o="Switch browse mode and helper mode.";break;case"showAxes":o="Axes line length: 50LY";break;case"showGrid":
o="Grid distance: 4LY";break;case"dcad":o="default Camera Animate Duration"}o&&(e.title=o)})}}(ConfigUI||(ConfigUI={}))
