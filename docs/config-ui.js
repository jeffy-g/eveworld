/*!
 ...
 - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
*/
import UniverseUnit from"eveworld/universe-unit.js";import Util from"eveworld/util.js"
;import{WorldConfig}from"eveworld/config.js";import View from"eveworld/view.js";function forEachRegioins(e){
const o=EVEWorld.getRegionObjects();for(const n of Object.values(o))e(n)}function forEachRegioinsMaterial(e){
const o=EVEWorld.getRegionObjects();for(const n of Object.values(o))e(n.material)}var ConfigUI;!function(e){
function o(e,o){const t=n(e);t&&(t.element.style.opacity=+o+"")}function n(e){
return e.getObjectByProperty("type","CSS2DObject")}e.initGUI=function(t){
const{camera:a,raycaster:i,scene:d,webGLRenderer:r}=View.getTHREERelated(),s=new lil.GUI({closeOnTop:!0,load:{
preset:"Default",close:!1,remembered:{Default:{0:Util.deepClone(WorldConfig)}}}});let l;s.load(WorldConfig),
s.add(WorldConfig,"helperMode").onChange((e=>{ThreeComponents.cameraHelper.visible=e})),
s.add(WorldConfig,"showAxes").onChange((e=>{ThreeComponents.axes.visible=e
})),s.add(WorldConfig,"showGrid").onChange((e=>{ThreeComponents.gridHelper.visible=e})),WorldConfig.resetCamera=()=>{
EVEWorld.defaultCameraPosition()},s.add(WorldConfig,"resetCamera"),s.add(WorldConfig,"updateRegionColors")
;const c=new THREE.Vector3;s.add(WorldConfig,"Find system").onChange((e=>{const o=EVEWorld.findSystemByName(e)
;o&&(console.log(o),EVEWorld.fireSystemHunted(o),EVEWorld.lookAt(c.set(...o.vec)))}))
;const g=s.addFolder("World Config#1").close();{function f(e,o){const n=d.getObjectByName(t);n&&n[e](...o)}
...
const m=s.addFolder("World Config#2");{const W=WorldConfig.worldConfig;m.add(W,"visible").onChange((e=>{
...
;const u=s.addFolder("Material config").close();u.add(WorldConfig,"Apply texture").onChange((e=>{
...
o.depthFunc=+e,o.needsUpdate=!0}))}));const p=s.addFolder("Region Visibility").close();{
...
}))}));const v=/\w-\w\d{5}/,T=p.addFolder("wormhole region").close(),U=p.addFolder("universe region").close(),w=[]
;T.add(b.wormhole,"ALL").onChange((e=>{for(const o of w)o.setValue(e)}));for(const x of R){
const V=v.test(x),H=V?b.wormhole:b.universe;H[x]=!0;const S=(V?T:U).add(H,x).onChange((e=>{const n=y[x];n&&(n.visible=e,
o(n,e))}));V&&w.push(S)}}const C=s.addFolder("Effect Sound").close();{const D=AppEffects.getDefaultTagList()
...
const h=s.addFolder("Render config").close()
;h.add(WorldConfig,"reduceFps"),h.add(WorldConfig,"PixelRatio").min(.1).max(3).step(.1).onChange((function(e){
r.setPixelRatio(e)}))},e.TagSystemHunt="tick",e.TagSystemCloseUp="fadein",e.addAnnotationToGui=function(){
